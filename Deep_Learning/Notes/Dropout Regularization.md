...
$$d_l = \text{np.random.rand}(a_3.\text{shape[0]}, a_3.\text{shape[1]}) < keep\_prop$$ If $keep\_prop = 0.8$ then there is a $80\%$ chance that the $d_l$ has value 1 and $20\%$ that it is zero. We also modify $a_l$ as follows: $$a_l = \text{np.multiply}(a_l, d_l)$$ $$a_l = \frac{a_l}{leep\_prop} $$ 
We divide by `keep_prob` to keep the **expected value of  unchanged** during training. This is called inverted dropout. 
$$E\Bigg[  \frac{a.d}{keep\_prop}  \Bigg] = a$$
